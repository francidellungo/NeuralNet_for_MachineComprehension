...
class BiDafModel(tf.keras.Model):
    def __init__(self, char_vocab_size, conv_filters=100, conv_filter_width=5, emb_dim=None):
        ...
        self.bi_lstm = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(self.lstm_units, return_sequences=True), merge_mode='concat')

        self.attention = AttentionLayer()

        self.modeling_layer1 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(self.lstm_units, return_sequences=True), merge_mode='concat')
        self.modeling_layer2 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(self.lstm_units, return_sequences=True), merge_mode='concat')

        self.output_layer = OutputLayer(self.lstm_units)
    ...
    def call(self, c_word_input, c_char_input, q_word_input, q_char_input):
        ...
        for i in range(len(c_word_input)):
            ...
            p_start, p_end = self.output_layer(M, G)
